namespace ConsoleClient
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            var categories = context.Categories.OrderByDescending(c => c.Books.Count).Select(c => new {c.Name, c.Books});

            //foreach (var category in categories)
            //{
            //    Console.WriteLine($"--{category.Name}: {category.Books.Count}");
            //    var mostRecentBooks = category.Books.OrderByDescending(b => b.ReleaseDate).ThenBy(b => b.Title).Take(3).Select(b=> new {b.Title, b.ReleaseDate} );
            //    foreach (var mostRecentBook in mostRecentBooks)
            //    {
            //        Console.WriteLine($"{mostRecentBook.Title} ({mostRecentBook.ReleaseDate})");
            //    }
            //}
