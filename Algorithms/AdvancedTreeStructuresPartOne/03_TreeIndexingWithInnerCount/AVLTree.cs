namespace _03_TreeIndexingWithInnerCount
{
    ...
    public class AvlTree<T> where T : IComparable<T>
    {
        ...
        public T this[int number]
        {
            get
            {
                if (number < 0 || number >= this.root.Count)
                {
                    throw new InvalidOperationException("ai spri sa malko???");
                }
                return FindNodeAtIndex(this.root, number);
            }
        }
        ...
        private void RetraceInsert(Node<T> node)
        {
            ...
            while (parent != null)
            {
                ...
                if (node.IsLeftChild)
                {
                    if (parent.BalanceFactor == 1 && !hasRotated)
                    {
                    ...
                    else if (parent.BalanceFactor == -1 && !hasRotated)
                    {
        ...
        public IEnumerable<T> Range(T from, T to)
        {
            ...
            elements = this.InternalRange(this.root, from, to, elements);
            return elements;
        }
        ...
        private List<T> InternalRange(Node<T> node, T from, T to, List<T> elements)
        {
            ...
            {
                this.InternalRange(node.LeftChild, from, to, elements);
            }
            ...
            {
                this.InternalRange(node.RightChild, from, to, elements);
            }
            return elements;
        }
        public T FindNodeAtIndex(Node<T> node, int index)
        {

            int leftSize = 0;
            if (node.LeftChild != null)
            {
                leftSize = node.LeftChild.Count;
            }
            if (leftSize == index)
            {
                return node.Value;
            ...
            {
                return this.FindNodeAtIndex(node.LeftChild, index);
            }
            else
            {
                return this.FindNodeAtIndex(node.RightChild, index - (leftSize + 1));
            }
        }
    }
}
