...
(function mathItUp() {
    ...
    let length = function (vec1) {
        let firstCoordinate = Math.sqrt((vec1[0] * vec1[0]) + (vec1[1] * vec1[1]));

        return firstCoordinate
    };

    let dot = function (vec1, vec2) {
        let firstCoordinate = (vec1[0] * vec2[0]) + (vec1[1] * vec2[1]);
        return firstCoordinate;
    };
    
    let cross = function (vec1,vec2) {
        let firstCoordinate = vec1[0] * vec2[1];
        let secondCoordinate = vec1[1] * vec2[0];
        return firstCoordinate - secondCoordinate;
    };
